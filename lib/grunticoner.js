...
(function(exports) {
	...
	var crusher = require( "./pngcrusher" );
	...
	//	type: "SVG" || "PNG"
	// }
	exports.getStats = function( imagedata , isSvg , filename , dir , o ){
	...
	//  height
	//  o
	exports.createCSSRules = function( cssData ){
		var imagedata = cssData.imagedata,
			pngimgstring = cssData.pngimgstring,
			filenamenoext = cssData.filenamenoext,
			isSvg = cssData.isSvg,
			width = cssData.width,
		...
		var getPNGDataCSSRule = function( prefix , pngimgstring ){
			if (pngimgstring.length <= 32768) {
				...
				return "/* Using an external URL reference because this image would have a data URI of " +
					...
					iconsel + " { background-image: url(" + o.pngout + "mod/" + filenamenoext + ".png" + "); background-repeat: no-repeat; }";
			}
		}; //getPNGDataCSSRule
		if( isSvg ) {
		...
		res.pngcssrule = iconsel + " { background-image: url(" + o.pngout + "mod/" + filenamenoext + ".png" + "); background-repeat: no-repeat; }";
	...
	exports.render = function( imagedata , filename , filenamenoext , isSvg , data , o) {
		var renderp = new RSVP.Promise();
		var self = this;
		var page = require( "webpage" ).create();
		var width = data.width, height = data.height, type = data.type;
		var pngDest = o.outputdir + o.pngout;
		var pngName = filenamenoext + ".png";
		...
		page.open(  o.inputdir + filename, function( status ){
			...
			if( status === "success" ){
				...
				// crush file
				crusher.crush( {
					input: pngDest + pngName,
					outputDir:  pngDest + "mod/",
					outputFilename:  pngName,
					crushPath: o.crushPath
				}, function(){
					p2 = require( "webpage" ).create();
					p2.viewportSize = {  width: parseFloat(width), height: parseFloat(height) };
					p2.open( pngDest + "mod/" + pngName , function( status ){
						var pngimgstring = p2.renderBase64( "png" );
						var cssData = {
							imagedata: imagedata,
							pngimgstring: pngimgstring,
	...
	exports.processFile = function( filename , o ){
		...
		self.getStats( imagedata , isSvg, filename , o.inputdir , o )
		.then( function( data ){
			return self.render( imagedata , filename , filenamenoext , isSvg , data , o );
		})
		.then( self.createCSSRules )
		.then( function( res ){
			promise.resolve( res );
		});
